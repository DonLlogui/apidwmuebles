...
class usuariosControlador {
  ...
  static async buscarPorDoc(req, res) {
    ...
    try {
      const users = await UsuarioModel.buscarPorDoc(doc);
      if (!users.length) {
        return res.status(404).json({ error: 'Usuario no encontrado' });
      }
      res.json(users);
  ...
  static async crearUsuario(req, res) {
    ...
    try {
      const result = await UsuarioModel.crearUsuarios(doc, name, tel, email, contra);
  ...
  static async editarUsuario(req, res) {
    ...
    const { doc, name, tel, email, contra } = req.body;
    try {
      const result = await UsuarioModel.modificarUsuario(id, doc, name, tel, email, contra);
